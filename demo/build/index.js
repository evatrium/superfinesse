...
const RECYCLED_NODE = 1,
  ...
  if (key[0] === '-') style.setProperty(key, value);else style[key] = typeof value === 'number' && IS_NON_DIMENSIONAL.test(key) === false ? value + 'px' : value == null ? '' : value;
},
  ...
  if (name === 'key' || name === 'children') ; else if (name === 'style') {
    _style = dom.style;
    if (typeof value === 'string') _style.cssText = value;else {
    ...
    } else if (value == null || value === false && !/^ar/.test(name)) dom.removeAttribute(name);else dom.setAttribute(name, value);
  }
},
  ...
  if (oldVNode === newVNode) {
    ...
    if (oldHead > oldTail) {
      while (newHead <= newTail) {
        node.insertBefore(createNode(newVKids[newHead++], isSvg), (oldVKid = oldVKids[oldHead]) && oldVKid.node);
      }
  ...
  var node = vnode.type === TEXT_NODE ? d.createTextNode(vnode.name) : (isSvg = isSvg || vnode.name === "svg") ? d.createElementNS("http://www.w3.org/2000/svg", vnode.name) : d.createElement(vnode.name);
  ...
  vdom = createVNode(oldVDom.name, EMPTY_OBJ, [].concat(vdom), oldVDom.node);
  return (node = patchNode(node.parentNode, node, oldVDom, vdom)).vdom = vdom, node;
},
  ...
  while (rest.length > 0) {
    ...
    } else if (vnode === false || vnode === true || vnode == null) ; else children.push(typeof vnode === 'object' ? vnode : createTextVNode(vnode));
  }

  return typeof name === 'function' ? (props.children = props.children || children) && name(props) : createVNode(name, props, children, null, props.key);
};
...
const App = () => {
  ...
  return h(Fragment, null, h("h1", null, "count: ", count), h(Button, {
    onClick: () => {
      console.log('click');
      update({
        count: count + 1
